...
uv run python tool.py check
uv run python tool.py audit
uv run python tool.py audit "проверь авторизацию, роли, поиск, фильтрацию, сортировку, CRUD, импорт и картинки"
cd ..
...
```powershell
cd tools
uv run python tool.py ask "Посмотри core/models.py и config/settings.py. Почему makemigrations падает?"
uv run python tool.py ask "Посмотри core/views.py, config/urls.py и core/templates/core/partner_list.html. Чего не хватает для списка?"
uv run python tool.py ask "Посмотри tools/output/manual_check.md и скажи, какие файлы исправлять первыми"
uv run python tool.py chat
uv run python tool.py restore
uv run python tool.py test_pages
```
...
`ask` всегда собирает общий контекст: файлы задания из `tools/input`, Excel-обзор, основные файлы проекта и последние отчеты из `tools/output`.

Если в вопросе явно написать имя файла, скрипт дополнительно прочитает этот файл и приложит его к вопросу. Можно писать коротко:
...
Можно писать абсолютный путь, если удобно:

```powershell
uv run python tool.py ask "Прочитай `C:\Users\Даниил\Desktop\test demo\app\core\admin.py` и объясни файл"
```

AI не имеет прямого доступа к диску. Он видит только то, что `tool.py` прочитал и отправил в запрос. Поэтому для точного ответа называй файлы прямо в вопросе.

## Django admin

`core/admin.py` нужен для встроенной админки Django. Он говорит Django, какие модели показывать на странице `/admin/`.

Чтобы открыть админку:
...
В админке удобно быстро проверить, что таблицы создались, импорт реально загрузил данные, связи работают, а модели не пустые. Обычный пользовательский интерфейс проекта от `admin.py` почти не зависит.
